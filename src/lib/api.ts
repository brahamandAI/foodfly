...
export const authApi = {
  login: async (email: string, password: string) => {
    try {
      ...
      document.cookie = `user=${JSON.stringify(user)}; path=/; max-age=${60 * 60 * 24 * 7}; SameSite=Lax`;
  ...
  getCurrentUser: async () => {
    try {
      const response = await axiosInstance.get('/api/auth/user-session');
...
// Authentication backup utility
export const backupAuthentication = () => {
  try {
    const token = localStorage.getItem('token');
    const user = localStorage.getItem('user');
    ...
    if (token && user && isLoggedIn === 'true') {
      ...
      // Also ensure cookies are set for middleware
      document.cookie = `token=${token}; path=/; max-age=${60 * 60 * 24 * 7}; SameSite=Lax`;
      document.cookie = `user=${user}; path=/; max-age=${60 * 60 * 24 * 7}; SameSite=Lax`;
      
      console.log('Authentication backed up to session storage and cookies');
      return true;
    }
    return false;
  } catch (error) {
    console.error('Failed to backup authentication:', error);
    return false;
  }
};
...
export const restoreAuthentication = () => {
  try {
    ...
    if (!token || !user || isLoggedIn !== 'true') {
      ...
      const backupIsLoggedIn = sessionStorage.getItem('isLoggedIn');
      
      if (backupToken && backupUser && backupIsLoggedIn === 'true') {
        localStorage.setItem('token', backupToken);
        localStorage.setItem('user', backupUser);
        localStorage.setItem('isLoggedIn', 'true');
        
        // Also restore cookies for middleware
        document.cookie = `token=${backupToken}; path=/; max-age=${60 * 60 * 24 * 7}; SameSite=Lax`;
        document.cookie = `user=${backupUser}; path=/; max-age=${60 * 60 * 24 * 7}; SameSite=Lax`;
        
        console.log('Authentication restored from session storage and cookies');
        return true;
      }
      return false;
    }
...
// Ensure authentication is available to middleware
export const ensureAuthInCookies = () => {
  try {
    const token = localStorage.getItem('token');
    const user = localStorage.getItem('user');
    const isLoggedIn = localStorage.getItem('isLoggedIn');
    ...
    if (token && user && isLoggedIn === 'true') {
      ...
      document.cookie = `token=${token}; path=/; max-age=${60 * 60 * 24 * 7}; SameSite=Lax`;
      document.cookie = `user=${user}; path=/; max-age=${60 * 60 * 24 * 7}; SameSite=Lax`;
      console.log('Authentication tokens set in cookies for middleware');
      return true;
    }
    return false;
  } catch (error) {
